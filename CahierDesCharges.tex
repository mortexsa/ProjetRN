...
Nous distinguons deux types d'apprentissages :
			\begin{itemize}
				\item \emph{Supervisé : }son fournit au réseau le couple (entrée, sortie attendue) et on modifie les poids en fonction de l'erreur entre la sortie désirée et la sortie obtenue.\\ \emph{Exemple : }utile aux chercheurs et aux ingénieurs qui disposent d'un ensemble de variables mesurées et d'un ensemble de mesure relative à un processus quelconque (physique, chimique, économique,financier...), du coup le but est d'établir un modèle du processus étudié à partir des mesures disponible.
				\item \emph{Non Supervisé : }le réseau doit détecter des points communs aux exemples présentés, et modifier les poids afin de fournir la même sortie pour des entrées aux caractéristiques proches.\\ \emph{Exemple : }utiles aux applications qui permettent de retrouver des informations dont sait qu'elles doivent être présente dans les données mais on ne sait pas comment les extraire.
			\end{itemize}
...
\section{Exigences Fonctionnelles}
	...
	\subsection{Organigramme et Liens entre Modules}
		...
		\paragraph{Connections entre les modules :}
			...
			\end{enumerate}
	\paragraph{Tableau des fonctionnalités \color{red}+ estimation en coût en ligne de code (690 lignes):}
	{\setlength{\tabcolsep}{0.07cm}
		...
		\color{red}(155) & \color{red}(150) & \color{red}(75) & \color{red}(310)\\
		\hline
		-Formatage des & -Algorithme de & -Récupération d'une & -Définition de la taille\\
		
		informations reçu par & rétropropagation \color{red}(50) & image au format & des couches cachéés du \\


		le module lecture/ & -Calcul de la fonction & variable \color{red}(25) & réseau de neurones \color{red}(15)\\	
		
		ecriture fichier \color{red}(50) & coût: \color{red}(25) & -récupération du & -attribution d'une \\

		-initialisation des & -calcul du gradient de &  couple image + & signification des neurones\\

		poids à la création du & la fonction coût afin & attribution d'une & de sortie \color{red}(10)\\

		réseau de neurones & d'adapter les poids du & étiquette \color{red}(50) & -choix du format\\

		\color{red}(15) & réseau de neurones & (les images récupérées & (BMP/MNIST)\color{red}(10)\\

		-reconnaissance du & \color{red}(50) & seront de format & -création/suppression/\\

		caractère présent sur & -calcul de la dérivée & BMP ou MNIST) & enregistrement d'un\\

		l'image \color{red}(50) & nécessaire pour la & & réseau de neurone \color{red}(10)\\

		-calcul matriciel & rétropropagation \color{red}(25) & & -possibilité de création de \\

		A = A x ... \color{red}(25) & & & plusieurs réseaux de\\
		-calcul sigmoid sur la & & & neurones et donc de\\

		matrice & & & sélection parmi plusieurs\\

		A = o(dsfs) \color{red}(15) & & & réseaux créés \color{red}(30)\\
		...
		& & & réseau de neurones (ex :\\
		& & & nombres de neurones\\
		& & & optimales etc...) \color{red}(10)\\
		& & & -mise en forme de\\
