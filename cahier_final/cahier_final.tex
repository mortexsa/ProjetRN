...
L'utilisation des champs de saisies pour la récupération des informations n'était pas évidente et nécessitait l'utilisation de fonctions plus avancées comme VBox ainsi que l'utilisation de plusieurs listes pour les récupérations. 
les fonctions Gtk nous permettait de récupérer qu'un seul argument et non pas plusieurs ce qui limitait l'interaction entre les différentes fenêtres.Pour résoudre ce probleme nous avons déclaré une structure INFO\_FENETRE pour stocker les variables comme window, ainsi que d’autres, ansi pouvoir recuperer toutes les informations circulant dans les fenêtres et les utiliser par la suite dans n'importe quelle fenetre.      
	\end{enumerate}
