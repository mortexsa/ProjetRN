...
L’estimation du coût a été un travail complexe. En effet, nous avons grandement sous-estimé le coût nécessaire à la création des modules de notre application. De plus, la gestion des problèmes lors de la programmation à augmenter le coût de nos modules étant déjà dévaluer lors de l’estimation. C’est ainsi que nous possédons un coût réel avec  un contraste important par rapport au coût initialement prévu.
...
W 1 et B 1 (appelé v 1 et v 2 sur le graphe ci-dessous). On peut représenter la fonction coût en fonction de ces
